...
def count_cond(condition):
    ...
    def condition_test(n):
        
            i, count = 1, 0
            while i <= n:
                if condition(n,i): ### if后的函数包含的lambda表达式已经包含了条件判定，所以这里可以直接写if cond（n）
                    count += 1
                i += 1
            return count
...
def cycle(f1, f2, f3):
    ...
    def times(n):
        def final_cycle(x):
            ## 双循环，一个值判定函数执行次数，一个值存储结果
            function_list = [f1, f2, f3]
            i, result = 0, x
            while i < n:
                num = i % 3
                result = function_list[num](result)
                i += 1
            return result
        return final_cycle
    return times
